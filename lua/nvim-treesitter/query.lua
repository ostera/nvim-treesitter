...
function M.get_capture_matches(bufnr, capture_string, query_group, root)
    ...
    local matches = {}
    for match in M.iter_group_results(bufnr, query_group, root) do
...
function M.iter_group_results(bufnr, query_group, root)
  ...
  if not lang then return EMPTY_ITER end
  ...
  if not lang_tree then return EMPTY_ITER end

  local query = M.get_query(lang_tree:lang(), query_group)
